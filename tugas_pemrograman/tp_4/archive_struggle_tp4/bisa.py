...
class MyBarcode(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.canvas = tk.Canvas(self, width=340, height=440, bg='white')
    ...
    def generate_barcode(self, checkdigit):
        ...
        numerical_part_checksum = barcode_num[1:] + str(self.checkdigit)
    ...
    def draw_barcode(self, barcode_patterns):
        ...
        for starting_line in range(2):
            ...
            self.canvas.create_line(
                10 + offset_starting_line, 10, 10 + offset_starting_line, 260, width=3, fill='blue'
            )

        barcode_position = 10 + offset_starting_line
        barcode_colors = ['white', 'black']
        for group_first in range(len(barcode_patterns)):
            self.canvas.create_line(
                barcode_position, 10, barcode_position, 260, width=3, fill=barcode_colors[int(barcode_patterns[group_first])]
            )
            barcode_position += 3
        ...
        for middle_line in range(2):
            self.canvas.create_line(
                barcode_position, 10, barcode_position, 260, width=3, fill='blue'
            )
            barcode_position += 6

        barcode_position -= 3
        for group_last in range(len(barcode_patterns)):
            barcode_position += 3
            self.canvas.create_line(
                barcode_position, 10, barcode_position, 260, width=3, fill=barcode_colors[int(barcode_patterns[group_last])]
            )
        ...
        self.canvas.create_text(
            barcode_position + 50, 280, fill='black', font='Arial 10 bold', text=self.check_digit
        )

        self.canvas.create_text(
            barcode_position + 50, 296, fill='orange', font='Arial 10 bold', text=f'Check Digit: {self.check_digit}'
        )
