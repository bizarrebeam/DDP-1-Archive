...
class MyBarcode(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.canvas = tk.Canvas(self, width=310, height=400, bg='white')
    ...
    def draw_barcode(self, barcode_patterns):
        ...
        for starting_line in range(2):
            ...
            self.canvas.create_line(
               30 + offset_starting_line, 30, 30 + offset_starting_line, 220, width=3, fill='blue'
            )
        ...
        for group_first in range(len(barcode_patterns) // 2):
            self.canvas.create_line(
                barcode_position, 30, barcode_position, 220, width=3, fill=barcode_colors[int(barcode_patterns[group_first])]
            )
            barcode_position += 3
        ...
        for middle_line in range(2):
            self.canvas.create_line(
                barcode_position, 30, barcode_position, 250, width=3, fill='blue'
            )
            barcode_position += 6
        ...
        for group_last in range(len(barcode_patterns) // 2, len(barcode_patterns) - 1):
            barcode_position += 3
            self.canvas.create_line(
                barcode_position, 30, barcode_position, 220, width=3, fill=barcode_colors[int(barcode_patterns[group_last])]
            )
        ...
        self.canvas.create_text(
            145, 235, fill='black', font='Courier 11 bold', text=f'{first_digit}\t{second_group}\t{third_group}'.expandtabs(2)
        )

        self.canvas.create_text(
           150, 270, fill='orange', font='Arial 16 bold', text=f'Check Digit: {self.check_digit}'
        )
