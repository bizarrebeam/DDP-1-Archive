...
def extract_attributes_content(lines):
    try: # Attempt to parse the attributes
        ...
        return xml_filename, provinsi, klasifikasi, \
            sub_klasifikasi, lembaga_peradilan
...
def scan_for_section(section, keyword_1, operator = None, keyword_2 = None):
    ...
    for xml_dataset in os.listdir("dataset"):
        ...
        xml_filename, provinsi, klasifikasi, sub_klasifikasi, \
            lembaga_peradilan = extract_attributes_content(xml_file)
        ...
        if start_line != 1 and end_line != -1:    
            ...
            if operator is None:                                   
                if keyword_1.lower() in xml_content:
                    total_match += 1
                    print(f"{xml_filename}{provinsi}{klasifikasi}\
                          {sub_klasifikasi}{lembaga_peradilan}")

            # Both keywords in that section's content                       
            elif operator == "AND":                     
                if keyword_1.lower() in xml_content and \
                        keyword_2.lower() in xml_content:
                    total_match += 1
                    print(f"{xml_filename}{provinsi}{klasifikasi}\
                          {sub_klasifikasi}{lembaga_peradilan}")
            ...
            # One of the keyword in that section's content
                ...
                if keyword_1.lower() in xml_content or \
                        keyword_2.lower() in xml_content:
                    total_match += 1
                    print(f"{xml_filename}{provinsi}{klasifikasi}\
                          {sub_klasifikasi}{lembaga_peradilan}")
            ...
            # The first keyword in the section but not the second keyword
                ...
                if keyword_1[0].lower() in xml_content and \
                        not keyword_2.lower() in xml_content:
                    total_match += 1
                    print(f"{xml_filename}{provinsi}{klasifikasi}\
                          {sub_klasifikasi}{lembaga_peradilan}")
    ...
    print(f"Total number of documents found\t= \
          {total_match}".expandtabs(10))
    print(f"Total search time\t= {scanning_time:.4f} \
          seconds".expandtabs(30))
...
def scan_entire_sections(keyword_1, operator = None, keyword_2 = None):
    ...
    for xml_dataset in os.listdir("dataset"):
        ...
        # Both keywords in that section's content                                    
            ...
            if keyword_1.lower() in xml_file and \
                    keyword_2.lower() in xml_file:
                total_match += 1
                print(f"{xml_filename}{provinsi}{klasifikasi}\
                      {sub_klasifikasi}{lembaga_peradilan}")
        ...
        # One of the keyword in that section's content
            ...
            if keyword_1.lower() in xml_file or \
                keyword_2.lower() in xml_file:
                total_match += 1
                print(f"{xml_filename}{provinsi}{klasifikasi}\
                      {sub_klasifikasi}{lembaga_peradilan}")
        ...
        # The first keyword in that section but not the second keyword
            ...
            if keyword_1[0].lower() in xml_file and \
                    not keyword_2.lower() in xml_file:
                total_match += 1
                print(f"{xml_filename}{provinsi}{klasifikasi}\
                      {sub_klasifikasi}{lembaga_peradilan}")
