...
# Function to display matched results on terminal
def display_matched(a, b, c, d, e):
    print(
        "| {: >45} | {: >15} | {: >15} | {: >30} | {: >20} ".format(
            a[:40], b[:15], c[:15], d[:30], e[:20]
        )
    )
    print(
        "| {: >45} | {: >15} | {: >15} | {: >30} | {: >20} ".format(
...
def scan_for_section(section, keyword_1, operator=None, keyword_2=None):
    ...
    print(
        f"Total search time\t= {scanning_time:.4f} seconds".expandtabs(40)
    )
...
def main(argv):
    ...
    # Assign the argument after validating
        ...
        # Calculate and display keyword frequencies
        for xml_dataset in os.listdir("dataset"):
            if xml_dataset.endswith(".xml"):  
                xml_path = os.path.join("dataset", xml_dataset)
                with open(xml_path, "r") as xml:
                    xml_file = xml.read().replace("\n", " ")
                    update_keyword_frequencies(xml_file)
