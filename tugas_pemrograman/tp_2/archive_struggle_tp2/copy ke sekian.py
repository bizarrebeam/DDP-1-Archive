...
def match_the_keywords(xml_file, section, keywords, operator):
    ...
    for line in open_the_xml(xml_file) : 
        ...
        if start_section in line:
            ...
            if operator == "AND" and a(keyword in current_section for keyword in keywords):
                matched = True
...
if __name__ == "__main__":
    ...
    section = input_args[0].strip("")
    operator = "AND"
    keywords = []

    if len(input_args) >= 2:
        operator = input_args[1]
        keywords = [k.strip('"') for k in input_args[2:]]
