...
def extract_tag(lines):
    try:                                                        # Attempt to parse the attributes
        ...
        words = lines.split()                                   # Split the whitespace into a single strings

        for word in words:
            if word.startswith("id="):
                xml_filename = word.split('"')[1] + ".xml"      # Split the attribute into 2 strings
            elif word.startswith("provinsi="):                      # and get the latter strings as output
                provinsi = word.split('"')[1]                       # that will be displayed
            elif word.startswith('klasifikasi="'):
                klasifikasi = word.split('"')[1]
            elif word.startswith('sub_klasifikasi="'):
                sub_klasifikasi = word.split('"')[1]
            elif word.startswith('lembaga_peradilan="'):
                lembaga_peradilan = word.split('"')[1]
    ...
    for xml_dataset in os.listdir("dataset"):
        if xml_dataset.endswith(".xml"):
            ...
            with open(xml_path, "r") as xml:
                ...
                # Access the variables for the attribute's cextracted content
                xml_filename, provinsi, klasifikasi, sub_klasifikasi, lembaga_peradilan = extract_tag(xml_file)
                ...
                if start_line != -1 and end_line != -1:    
                    ...
                    if operator is None:                                   
                        if keyword_1.lower() in xml_content:
                            total_match += 1
                            display_matched(xml_filename, provinsi, klasifikasi, sub_klasifikasi, lembaga_peradilan)
