...
def print_headers():
    print()
    print(
        "|{: >36}|{: >15}|{: >15}|{: >30}|{: >20}|{: >75}|".format(
        "Nama File ", "Provinsi ", "Klasifikasi ", "Sub-klasifikasi ", "Lembaga Peradilan ", "Potongan File " 
        )
    )
    print(
        "|{: >36}|{: >15}|{: >15}|{: >30}|{: >20}|{: >75}|".format(
...
def scan_for_section(section, keyword_1, operator=None, keyword_2=None):
    ...
    total_xmls = len(os.listdir("dataset"))                  # Feature: help tracks the percentage of files scanned
    ...
    for index, xml_dataset in enumerate(os.listdir("dataset")):                 
        ...
        with open(xml_path, "r") as xml:                     # Read the xml file
            ...
            if start_tag != -1 and end_tag != -1:                                           # Making sure each tag placed properly
                ...
                if (
                    ...
                    # Feature: Extract the last word of the keyword
                    last_keyword = keyword_1.lower().split()[-1]                            
                    keyword_index = xml_content.lower().rfind(last_keyword)

                    # Feature: Extract words after the last keyword
                    end_word_index = min(
                        len(xml_content), keyword_index + len(last_keyword) + 75            # The extracted snippets shouldn't be longer
                        )                                                                     # than the file's line length          
                    matched_word = xml_content[keyword_index:end_word_index]

                    display_matched(                                                        # Display the file's info on terminal
                        attributes["id"] + ".xml",
                        attributes["provinsi"],
        ...
        # Feature: Progress percentage to track the current scanning process
    ...
    for index, xml_dataset in enumerate(os.listdir("dataset")):
        ...
        with open(xml_path, "r") as xml:                    # Read the xml file
            ...
            if (
                ...
                # Extract the last word of the keyword
                last_keyword = keyword_1.lower().split()[-1]
                keyword_index = xml_file.lower().rfind(last_keyword)

                # Extract three words after the last keyword
                end_word_index = min(len(xml_file), keyword_index + len(last_keyword) + 75)
                matched_word = xml_file[keyword_index:end_word_index]
...
def main(argv):
    ...
    # The argument shouldn't be shorter or longer
    ...
    # Assign the argument after validating
        ...
        # Making sure user search within existing section
        valid_sections = [
