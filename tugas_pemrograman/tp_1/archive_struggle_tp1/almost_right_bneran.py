...
total_tower_width = (tower_amount-1) * (layer_width * one_brick_width + tower_distance * one_brick_width)
...
for tower in range(tower_amount):
    ...
    for layer in range(first_tower_layer):
        ...
        at_y += one_brick_width  # Move up one layer
    ...
    for i in range(layer_width + 1):
        ...
        for side in range(4):
            mario.forward(one_brick_width)
            mario.right(90)
        mario.end_fill()  # Add this line to fill the color
        if i < layer_width:
...
# Adjust the starting position for the next layer in the body
