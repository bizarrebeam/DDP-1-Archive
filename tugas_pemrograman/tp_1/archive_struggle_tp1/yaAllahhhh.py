...
while not valid:
    ...
    if tower_amount is None: 
        messagebox.showerror("The value is none :(", "Please provide a value.")
        continue

    if not tower_amount.isdigit(): 
        messagebox.showerror("Invalid input :(", "The amount of tower must be a positive integer. Please re-enter the input")
        continue
    
    tower_amount = int(tower_amount)

    if tower_amount < 1: 
        messagebox.showerror("Invalid input :(", "The amount of towers must be at least 1. Please re-enter the input")
        continue
...
if tower_amount > 1: 
    ...
    while not valid:
        ...
        if not tower_distance.isdigit(): 
            messagebox.showerror("Invalid input :(", "The distance between towers must be a positive integer. Please re-enter the input")
            continue

        tower_distance = int(tower_distance)

        if tower_distance < 2 or tower_distance > 5:
            messagebox.showerror("Invalid input :(", "The distance between towers must be between 2 and 5. Please re-enter the input")
            continue
    ...
    while not valid:
        ...
        if not layer_differences.isdigit(): 
            messagebox.showerror("Invalid input :(", "The number of layer differences must be a positive integer. Please re-enter the input")
            continue

        layer_differences = int(layer_differences)

        if layer_differences < 2 or layer_differences > 5:
            messagebox.showerror("Invalid input :(", "The number of layer differences must be between 2 and 5. Please re-enter the input.")
            continue
...
while not valid:
    ...
    if not one_brick_width.isdigit(): 
        messagebox.showerror("Invalid input :(", "The width of brick must be a positive integer. Please re-enter the input")
        continue

    one_brick_width = int(one_brick_width)

    if one_brick_width > 35:
        messagebox.showerror("Ups, it exceeds :(", "The maximum value for the width is 35. Please re-enter the input!")
...
while not valid:
    ...
    if not one_brick_height.isdigit(): 
        messagebox.showerror("Invalid input :(", "The height of brick must be a positive integer. Please re-enter the input")
        continue

    one_brick_height = int(one_brick_height)

    if one_brick_height > 25:
        messagebox.showerror("Ups, it exceeds :(", "The maximum value for the height is 25. Please re-enter the input!")
...
while not valid:
    ...
    if not first_tower_layer.isdigit(): 
        messagebox.showerror("Invalid input :(", "The amount of layer must be a positive integer. Please re-enter the input")
        continue

    first_tower_layer = int(first_tower_layer)

    if first_tower_layer > 25:
        messagebox.showerror("Ups, it exceeds :(", "The maximum value for the amount of layer is 25. Please re-enter the input!")
...
while not valid:
    ...
    if not layer_width.isdigit():
        messagebox.showerror("Invalid input :(", "The width of layer must be a positive integer. Please re-enter the input")
        continue

    layer_width = int(layer_width)

    if layer_width > 10:
        messagebox.showerror("Ups, it exceeds :(", "The maximum value for the width is 10 bricks. Please re-enter the input!")
    
    else:
        valid = True

# Calculate the total width of all towers and the spacing between them for position adjustment
total_tower_width = (tower_amount) * (layer_width + tower_distance) * (one_brick_width)

# Calculate the total height of all towers for position adjustment
...
for tower in range(tower_amount):
    ...
    cap_x = at_x + (layer_width * (one_brick_width) // 2) + (3 *one_brick_width // 2)
    ...
    mario.end_fill()
    # Move to the next mushroom in the next tower
    cap_x += (layer_width * one_brick_width // 2 + tower_distance + one_brick_width // 2)
