...
begin_x = -(tower_amount - 1) * (layer_width + tower_distance) * one_brick_width / 2 
begin_y = -(tower_amount - 1) * (layer_differences * one_brick_height) / 2
...
for tower in range(tower_amount):
    ...
    begin_y = -(tower_amount - 1) * (layer_differences * one_brick_height) / 2  # Reset the vertical position

    # Increment the initial layer width for the next tower
    layer_width += layer_differences
