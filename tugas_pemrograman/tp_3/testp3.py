...
def read_csv(file_name, delimiter = ','):
  ...
  """
  data = []                                                                                # The first element of the tuple is a list of list
    ...
    with open(file_name, "r") as file:
      ...
      for line_count, line in enumerate(file, start=1):                                    # Start tracking from line 1
        ...
        if line_count == 1:                                                                # The first line is guaranteed to be a header
          ...
          col_types = ['str'] * len(contents)                                              # The number of columns == len of header
          ...
          if not contents:                                                                 # If the line doesn't contain any value (blank)
            raise Exception(
               f"Tabel pada baris {line_count + 1} tidak boleh kosong."
               )
...
def select_rows_advanced(dataframe, conditions, order_by=None, ascending=True):          # Default parameter: ascending
    """
    ...
    for row in to_list(dataframe):                                                       # Check and returns boolean
        criteria = all(                                                                  
            satisfy_cond(row[dataframe[1].index(cond["col_name"])], cond["operator"], cond["value"])
            for cond in conditions
        )

        if criteria:                                                                     # Select specific criteria passed (True) as an argument
            selected_rows.append(row)

    for col_name in order_by:
        index_order_by = dataframe[1].index(col_name)
        selected_rows.sort(key=lambda x: x[index_order_by], reverse=not ascending)       # Perform sorting (key, reverse) in place. Set reverse in contrast with argument
...
def count(dataframe, col_name):
  ...
  frequency_count = {value: 0 for value in unique_values}                               # Initialize the dictionary compre--col_name as key, value is the count
...
if __name__ == "__main__":
    ...
    # Bonus
    conditions = [
    {"col_name": "Length", "operator": ">", "value": 0.3},
    {"col_name": "Sex", "operator": "==", "value": "I"},
    {"col_name": "Rings", "operator": ">=", "value": 3}
    ]
    ...
    print(head(new_4th_dataframe, top_n=3))
