...
def count(dataframe, col_name):
    ...
    if len(to_list(dataframe)) == 0:  # Proceed only if the dataframe is not empty
        raise Exception("Tabel kosong.")

    unique_values = set(row[get_column_names(dataframe).index(col_name)] for row in to_list(dataframe))

    # Initialize the dictionary -- col_name as key, value is the count
    ...
    for row in to_list(dataframe):
        ...
        frequency_count[value] += 1  # Increment the count for the value encountered
