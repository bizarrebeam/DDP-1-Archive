...
def select_rows(dataframe, col_name, condition, value):
  ...
  selected_rows = [row for row in to_list(dataframe) if satisfy_cond(row[index_col], condition, value)] # From the 1st element of tuple, perform matching operators
...
def scatter(dataframe, col_name_x, col_name_y):
    ...
    x_values = [float(row[x_index]) for row in to_list(dataframe)]                    # Convert it into float
    y_values = [float(row[y_index]) for row in to_list(dataframe)]                    # Convert it into float
