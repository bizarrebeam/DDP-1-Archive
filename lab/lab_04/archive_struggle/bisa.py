...
# Function to display the entire table content
def print_table(txtfile: str):
    try:    # Attempt to open the file
        with open(txtfile, "r") as file:
            ...
            # Numbering each data in txt file to adjust table formatting
            for index, line in enumerate(lines, start = 1):

                # Get the content file so it readable to the program
                content = line.strip().split('\t')

                # Print the content into table formatting
...
def desc_stat(txtfile: str, column: int):
    try:    # Attempt to open the file
        with open(txtfile, "r") as file:
            ...
            min_stat = float("inf")         # Minimal data that'd be displayed
            max_stat = float("-inf")        # Maximal data that'd be displayed 
            ...
            for line in lines:
                ...
                try:
                    ...
                    # Determine the min and max data
